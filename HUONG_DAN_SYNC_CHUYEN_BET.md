...
| Nhu cầu bắt đầu | `source_record_id` | Ghép thành `XN3:<Nhu cầu>` để không phụ thuộc STT/dòng Sheet |
| — | `source_system` | `flat_line_sheet` |
| — | `don_vi` | `XN3` theo cấu hình nguồn |
| Tổ | `bo_phan` | `U3 - L7` → `["Tổ 7"]`; `U3 - L1+8` → `["Tổ 1", "Tổ 8"]` |
...
FLAT_LINE_XN2_SPREADSHEET_ID=
FLAT_LINE_XN1_SPREADSHEET_ID=
FLAT_LINE_AUTO_SYNC_ENABLED=false
FLAT_LINE_AUTO_SYNC_INTERVAL_MINUTES=60
```

Khi có Sheet XN2/XN1, điền ID tương ứng. QLCL quy đổi alias `XN1` thành đơn vị
hiện hành `XN1-V1`.

## Cách chạy

- QAQT: vào trang Kế hoạch sản xuất, bấm `Đồng bộ Chuyền bệt XN3`.
